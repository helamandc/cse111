...
# Original code from BYU and updated to calculate the Area of a Circle given the radius inputted by the user
...
def populate_main_window(frm_main):
    ...
    Return: nothing
    """
    # Create a label that displayes "Input the radius of the circle":
    lbl_radius = Label(frm_main, text="Input radius of the circle:")

    # Create a float entry box where the user will enter the radius.
    ent_radius = IntEntry(frm_main, width=4, lower_bound=1, upper_bound=1000000)

    # Create a label that displays "meters"
    lbl_radius_units = Label(frm_main, text="meter")

    # Create a label that displays "Area of Circle:"
    lbl_area = Label(frm_main, text="Area of Circle:")

    # Create labels that will display the results.
    lbl_area_value = Label(frm_main, width=3)
    lbl_area_units = Label(frm_main, text="meter\u00B2")
    ...
    lbl_area_value.grid(      row=1, column=2, padx=3, pady=3)
    ...
    def calculate(event):
        ...
        try:
            ...
            # Compute the area by multiplying the pi value with square of the radius
